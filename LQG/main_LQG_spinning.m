function [pos_std, cur_std] = main_LQG_spinning(Om_s)
...
Om = 2*pi*(15000)/60; % [rads^-1] Spin velocity model
Om_s = 2*pi*(Om_s)/60; % [rads^-1] Spin velocity system
ts = 5e-5; % [s] Step size
...
dvx =   @(t,x,vx,y,vy,z,vz,phi,ophi,psi,opsi,ia1x,ia2x,ia1y,ia2y) (1/m)*(5*cos(Om_s*t)+ki*ia1x+ki*ia2x-2*ks*x+kpr*(x-lp*sin(psi)));
dy =    @(t,y,vy,z,vz,phi,ophi,psi,opsi,x,vx,ia1x,ia2x,ia1y,ia2y) vy;
dvy =   @(t,y,vy,z,vz,phi,ophi,psi,opsi,x,vx,ia1x,ia2x,ia1y,ia2y) (1/m)*(5*sin(Om_s*t)+ki*ia1y+ki*ia2y-2*ks*y+kpr*(y+lp*sin(phi)*cos(psi)));
...
dophi = @(t,phi,ophi,psi,opsi,x,vx,y,vy,z,vz,ia1x,ia2x,ia1y,ia2y) (1/I)*(-Izz*Om_s*opsi+l*(-ki*ia1y+ki*ia2y-2*ks*l*sin(phi)*cos(psi))+kpr*lp*(y+lp*sin(phi)*cos(psi)));
dpsi =  @(t,psi,opsi,x,vx,y,vy,z,vz,phi,ophi,ia1x,ia2x,ia1y,ia2y) opsi;
dopsi = @(t,psi,opsi,x,vx,y,vy,z,vz,phi,ophi,ia1x,ia2x,ia1y,ia2y) (1/I)*(Izz*Om_s*ophi+l*(ki*ia1x-ki*ia2x-2*ks*l*sin(psi))-kpr*lp*(x-lp*sin(psi)));
...
%% LQG as augmented system combining Kalman filter and LQR

A_ = [eye(4) zeros(4,40);
    zeros(40,4) A-Kf*C];

B_ = [ts*eye(4) -ts*[eye(4) zeros(4)] zeros(4);
    zeros(40,4) Kf B];
...
[BR,G,~,T] = balreal(ss(A_,B_,C_,D_,ts));
BR = modred(BR,31:44);
% Size of experimental states
sz_exp = 30;
dif_sz = sz_exp-length(BR.a);
A_ = [BR.a zeros(length(BR.a),dif_sz);
    zeros(dif_sz,sz_exp)];
B_ = [BR.b;zeros(dif_sz,size(BR.b,2))];
C_ = [BR.c zeros(size(C_,1),dif_sz)];
...
for k = 1:length(sim_length)
    ...
    % Control Law
    AX = A_*X_;
    AX = min(1.64e4*ones(30,1),max(-1.64e4*ones(30,1),AX));
    BU = B_*[Yref;Y(1:4)*10^6;Y(5:end);i_dp];
    BU = min(1.64e4*ones(30,1),max(-1.64e4*ones(30,1),BU));
    CX = C_*X_;
    CX = min(2*ones(4,1),max(-2*ones(4,1),CX));
...
%     end
    ...
    St = [St [x vx y vy phi ophi psi opsi ia1x ia1y ia2x ia2y]'];
end
